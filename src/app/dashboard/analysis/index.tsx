...
export default function AnalisisPage() {
    ...
    useEffect(() => {
        ...
        if (user?.country_of_residence) {
            const region = Object.keys(regionMap).find((key) =>
                regionMap[key as keyof typeof regionMap].includes(
                    user.country_of_residence.toLowerCase()
                )
            );
    ...
    const isStep2Complete = () => {
        ...
        let allComplete = requiredParams.every(
            (param) =>
                formData.healthProfile[
                    param as keyof typeof formData.healthProfile
                ]?.completed
        );
    ...
    const updateHealthParameter = (
        param: string,
        updates: Partial<HealthParameter>
    ) => {
        setFormData((prev) => ({
            ...prev,
            healthProfile: {
                ...
                [param]: {
                    ...prev.healthProfile[
                        param as keyof typeof prev.healthProfile
                    ],
                    ...updates,
                },
            },
            ...
            <Card className="bg-white shadow-md hover:shadow-lg transition-all duration-300 rounded-2xl border border-gray-200">
                <CardContent className="p-4 md:p-6 lg:p-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-3">
                            <Label className="text-sm font-medium text-gray-700">
                                Wilayah Risiko Geografis Anda
                            </Label>
                            <Select
                                value={formData.riskRegion}
                                onValueChange={(value) =>
                                    updateFormData("riskRegion", value)
                                }
                                disabled
                            >
                                <SelectTrigger className="w-full rounded-md border border-gray-300 focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-all duration-300 h-10 md:h-12 text-base">
                                ...
                                <SelectContent>
                                    <SelectItem value="Risiko Rendah">
                                        Risiko Rendah
                                    </SelectItem>
                                    <SelectItem value="Risiko Sedang">
                                        Risiko Sedang
                                    </SelectItem>
                                    <SelectItem value="Risiko Tinggi">
                                        Risiko Tinggi
                                    </SelectItem>
                                    <SelectItem value="Risiko Sangat Tinggi">
                    ...
                    <div className="space-y-3">
                        ...
                        <RadioGroup
                            ...
                            onValueChange={(value) =>
                                updateFormData("diabetesHistory", value)
                            }
        ...
        >
            {healthParameters.map((param, index) => (
                <HealthParameterCard
                    ...
                    data={
                        formData.healthProfile[
                            param.key as keyof typeof formData.healthProfile
                        ]
                    }
                    onUpdate={(updates: Partial<HealthParameter>) =>
                        updateHealthParameter(param.key, updates)
                    }
                    index={index}
...
function HealthParameterCard({ parameter, data, onUpdate, index, total }: any) {
    ...
    const handleProxyAnswerChange = (questionKey: string, value: any) => {
        ...
        const allAnswered = parameter.proxyQuestions.every(
            (q: any) =>
                newProxyAnswers[q.key] !== undefined &&
                newProxyAnswers[q.key] !== ""
        );
