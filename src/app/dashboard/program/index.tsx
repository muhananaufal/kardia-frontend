...
export default function CoachingDashboard() {
    ...
    const [graduationDetails, setGraduationDetails] =
        useState<GraduationDetails | null>(null);
    ...
    const [selectedWeek, setSelectedWeek] = useState(
        programData?.overall_progress.current_week_number
    );
    ...
    useEffect(() => {
        const loadProgramDetails = async () => {
            ...
            try {
                ...
                if (!slugToFetch) {
                    const programOverview = await fetchDashboard(token).then(
                        (data) => data.data?.program_overview
                    );
                ...
                if (slugToFetch) {
                    ...
                    if (response && response.data) {
                        ...
                        setSelectedWeek(
                            response.data.overall_progress.current_week_number
                        );
                ...
                if (programStatus === "completed") {
                    const graduationResponse = await getGraduationDetails(
                        token,
                        programData?.slug ?? ""
                    );
    ...
    const currentWeekData = programData?.weeks.find(
        (week) => week.week_number === selectedWeek
    );
    ...
    const isReadOnly =
        isProgramInactive ||
        (selectedWeek ?? 0) < programData?.overall_progress.current_week_number;

    // Tentukan minggu yang sudah selesai untuk di-highlight di timeline
    const completedWeeks = programData?.weeks
        .filter(
            (week) =>
    ...
    const handleMissionComplete = async (id: string) => {
        ...
        const weekToUpdate = newProgramData.weeks.find(
            (week: any) => week.week_number === selectedWeek
        );
            ...
            if (weekToUpdate) {
                ...
                await updateCompletionMissions(token, id)
                    .then(() => {
                        ...
                        const completedTasks = weekToUpdate.tasks.filter(
                        ...
                        weekToUpdate.completion_percentage =
                            completionPercentage;

                        const overallCompletedTasks =
                            newProgramData.weeks.reduce(
                                (acc: number, week: any) => {
                                    return (
                                        acc +
                                        week.tasks.filter(
                        ...
                        const overallTotalTasks = newProgramData.weeks.reduce(
                            (acc: number, week: any) => {
                        ...
                        newProgramData.overall_progress.overall_completion_percentage =
                            Math.round(
    ...
    const groupedTasks = useMemo(() => {
        ...
        return currentWeekData.tasks.reduce(
            (acc: { [key: string]: any[] }, task: any) => {
                        ...
                        <div className="flex items-center justify-center gap-2 my-4">
                            ...
                            >
                                ...
                                <span className="text-sm font-medium">
                                    Program{" "}
                                    {getProgramStatusLabel(programData.status)}
                                </span>
                            </Badge>
                        </div>
                    </div>
                    ...
                    <CardContent>
                        <div className="space-y-3">
                            ...
                            <p className="text-sm text-slate-600">
                                {programData.overall_progress.days_remaining}{" "}
                                hari tersisa. Kamu Hebat!
                            </p>
                        </div>
                    </CardContent>
                </Card>
                            ...
                            <>
                                ...
                                groupedTasks[today].length > 0 ? (
                                    ...
                                    <p className="text-slate-500">
                                        Tidak ada tugas untuk hari ini. Selamat
                                        beristirahat!
                                    </p>
                                )}
                            </>
                                ...
                                {Object.keys(groupedTasks).length > 0 ? (
                                    Object.entries(groupedTasks).map(
                                        ([date, missions]) => (
                                            <div
                                                key={date}
                                                className="mb-6 border rounded-lg p-6"
                                            >
                                                <h4 className="font-semibold pb-2 mb-3">
                        ...
                        <CardContent>
                            <div className="flex flex-col items-center gap-4">
                                <p className="text-center text-violet-700">
                                    Anda telah berhasil menyelesaikan program
                                    ini. Lihat laporan kelulusan Anda untuk
                                    melihat ringkasan pencapaian.
                                </p>
                                <Dialog
                                    open={showGradReport}
                                    onOpenChange={setShowGradReport}
                                >
                                    <DialogTrigger asChild>
                                        <Button
                                            className="bg-violet-600 hover:bg-violet-700 w-full"
                                            disabled={!graduationDetails}
                                        >
                                            {graduationDetails ? (
                                                <>
                        ...
                        <CardContent>
                            <div className="flex flex-col items-center gap-4">
                                <Dialog>
                                    <DialogTrigger
                                        asChild
                                        className="justify-start w-full h-[40px] text-gray-600 border border-gray-300 hover:bg-gray-50 rounded-lg text-sm font-medium cursor-pointer"
                                    >
                                        <Button
                                            variant="outline"
                                            size="sm"
                                    ...
                                    <DialogContent>
                                        <DialogHeader>
                                            ...
                                            <DialogDescription>
                                                ...
                                                your program? You can resume it
                                                later.
                                            </DialogDescription>
                                        </DialogHeader>
                                        <DialogFooter>
                                ...
                                <Dialog>
                                    <DialogTrigger
                                        asChild
                                        className="justify-start w-full h-[40px] text-red-600 border border-red-300 hover:bg-red-50 rounded-lg text-sm font-medium cursor-pointer"
                                    >
                                        <Button
                                            variant="outline"
                                            size="sm"
                                    ...
                                    <DialogContent>
                                        <DialogHeader>
                                            ...
                                            <DialogDescription>
                                                ...
                                                your program? This action cannot
                                                be undone.
                                            </DialogDescription>
                                        </DialogHeader>
                                        <DialogFooter>
