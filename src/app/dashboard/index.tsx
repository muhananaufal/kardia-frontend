	...
	if (active && payload && payload.length) {
		...
		return (
			<div className="bg-white p-3 rounded-xl shadow-lg border border-slate-200">
				<p className="text-sm font-medium text-slate-800">{data.fullDate}</p>
...
export default function DashboardPage() {
	...
	return (
		<motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.5 }} className="p-6 space-y-6">
			{/* Header */}
			<motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="space-y-2">
				...
				<p className="text-slate-600">Pantau kondisi kesehatan Anda dengan analisis AI terdepan</p>
			</motion.div>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
					...
					<Card className="gap-3 rounded-2xl shadow-sm border h-full">
						<CardHeader>
							<div className="flex items-center justify-between">
								<CardTitle className="text-lg text-rose-600">Status Kesehatan</CardTitle>
						...
						<CardContent>
							<div className="space-y-2">
								{dashboardData?.summary.latest_status.category_code ? (
									formatResikoBadge(dashboardData?.summary.latest_status.category_code, dashboardData?.summary.latest_status.category_title)
								) : (
									<Badge className="bg-gray-100 text-gray-600">Tidak ada status kesehatan</Badge>
								)}
								<p className="text-sm">{dashboardData?.summary.latest_status.description || 'Tidak ada deskripsi status kesehatan.'}</p>
							</div>
						</CardContent>
					</Card>
				</motion.div>
				...
				<motion.div variants={cardVariants} initial="initial" animate="animate" transition={{ delay: 0.3 }}>
					<Card className="gap-3 rounded-2xl shadow-sm border h-full">
						<CardHeader>
							<div className="flex items-center justify-between">
								<CardTitle className="text-lg text-rose-600">Analisis Terakhir</CardTitle>
								<Clock className="h-6 w-6 text-rose-600" />
							</div>
						</CardHeader>
						<CardContent>
							<div className="space-y-2">
								<p className="text-xl font-bold">{dashboardData?.summary.last_assessment_date_human || 'Belum ada analisis'}</p>
				...
				<motion.div variants={cardVariants} initial="initial" animate="animate" transition={{ delay: 0.4 }}>
					<Card className="gap-3 rounded-2xl shadow-sm border h-full">
						<CardHeader>
							<div className="flex items-center justify-between">
								<CardTitle className="text-lg text-rose-600">Trend Kesehatan</CardTitle>
								<TrendingUp className="h-6 w-6 text-rose-600" />
							</div>
						</CardHeader>
						<CardContent>
							<div className="space-y-2">
								<p className="text-2xl font-bold">{dashboardData?.summary.health_trend.direction ? getTrendStatus(dashboardData?.summary.health_trend.direction).text : 'Tidak ada data'}</p>
								<p className="text-sm">{dashboardData?.summary.health_trend.text || 'Tidak ada informasi tren kesehatan.'}</p>
							</div>
						</CardContent>
					</Card>
				</motion.div>
						...
						<div className="h-40 md:h-60 w-full">
							<ResponsiveContainer width="100%" height="100%">
								...
								>
									...
									<YAxis stroke="#64748b" fontSize={12} tickLine={false} axisLine={false} domain={[0, 30]} tickFormatter={(value) => `${value}%`} />
					...
					<Card className="rounded-2xl shadow-md px-0 py-6 md:px-6 md:py-6 hover:shadow-lg transition-all duration-300 bg-white border">
						...
						<CardContent className="space-y-6">
							...
							<div className="flex items-center gap-3 p-4 rounded-xl bg-white/60 border border-rose-200/50">
								...
								<div>
									<p className="font-medium text-rose-800">Terakhir melakukan analisis: </p>
									<p className="text-sm text-rose-700">{dashboardData?.summary.last_assessment_date_human}</p>
								</div>
							</div>
							...
							<div className="flex items-start gap-3 p-4 rounded-xl bg-yellow-100/60 border border-yellow-200">
								...
								<div className="flex-1">
									<p className="font-medium text-yellow-800 mb-1">Rekomendasi Sistem</p>
									<p className="text-sm text-yellow-700 leading-relaxed">
										Sudah cukup lama sejak analisis terakhir, sebaiknya lakukan pengecekan kembali untuk mengetahui kondisi terkini Anda. Analisis rutin membantu memantau perubahan kesehatan secara dini.
									</p>
								</div>
							</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div className="p-4 rounded-xl bg-white/60 border border-rose-200/50">
									<h4 className="font-medium text-slate-800 mb-2">Hasil Terakhir</h4>
									<div className="space-y-2">
										<div className="flex justify-between items-center">
											<span className="text-sm text-slate-600">Status Risiko</span>
											{formatResikoBadge(dashboardData?.latest_assessment_details.riskSummary.riskCategory.code, dashboardData?.latest_assessment_details.riskSummary.riskCategory.title)}
										</div>
										<div className="flex justify-between items-center">
											<span className="text-sm text-slate-600">Persentase Resiko</span>
											<span className="font-semibold text-slate-800">{dashboardData?.latest_assessment_details.riskSummary.riskPercentage}%</span>
										</div>
									</div>
								</div>

								<div className="p-4 rounded-xl bg-white/60 border border-rose-200/50">
									<h4 className="font-medium text-slate-800 mb-2">Faktor Utama</h4>
									<ul className="text-sm text-slate-600 space-y-1">
										{dashboardData?.latest_assessment_details.riskSummary.primaryContributors.slice(0, 3).map((item, i) => (
