...
export default function RiwayatPage() {
    ...
    const [programDifficulty, setProgramDifficulty] = useState<
        "Santai & Bertahap" | "Standar & Konsisten" | "Intensif & Menantang"
    >("Standar & Konsisten");
        ...
        const loadDashboardData = async () => {
            ...
            try {
                ...
                if (
                    responseData &&
                    responseData.data &&
                    Array.isArray(responseData.data.assessment_history)
                ) {
                    setAnalysisHistory(responseData.data.assessment_history);
                } else {
                    console.error(
                        "Path 'data.assessment_history' tidak ditemukan atau bukan array:",
                        responseData
                    );
                    setAnalysisHistory([]);
                }
    ...
    const filteredHistory = analysisHistory
        .filter((record) => {
            ...
            const matchesFilter =
                filterRisk === "all" ||
                getRiskLevel(
                    record?.result_details?.riskSummary?.riskCategory?.code
                ) === filterRisk;
        ...
        <div className="min-h-screen bg-white">
            ...
            >
                ...
                >
                    ...
                    <p className="text-base md:text-lg text-gray-600 leading-relaxed">
                        Pantau perkembangan kesehatan jantung Anda dari waktu ke
                        waktu
                    </p>
                </motion.div>
                <motion.div
                            ...
                            <p className="text-2xl md:text-3xl font-bold text-gray-900">
                                {(() => {
                                    ...
                                    analysisHistory.forEach((record) => {
                                        ...
                                        if (!monthlyRisks[month])
                                            monthlyRisks[month] = [];
                                        monthlyRisks[month].push(
                                            record.risk_percentage || 0
                                        );
                                    });

                                    const totalMonthlyRisk = Object.values(
                                        monthlyRisks
                                    ).reduce(
                                        (acc, risks) =>
                                            acc +
                                    ...
                                    const totalMonthlyCount = Object.values(
                                        monthlyRisks
                                    ).reduce(
                                        (acc, risks) => acc + risks.length,
                                        0
                                    );

                                    return totalMonthlyCount > 0
            ...
            <Dialog open={showConfirmModal} onOpenChange={setShowConfirmModal}>
                <DialogContent className="sm:max-w-md">
                    ...
                    <div className="space-y-4">
                        <p className="text-slate-600">
                            Kamu akan memulai program baru. Program aktif saat
                            ini akan dibatalkan. Apakah kamu yakin ingin
                            melanjutkan?
                        </p>
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                placeholder="Ketik LANGKAH BARU"
                                className="rounded-md border-slate-300 focus:border-red-500 focus:ring-red-500 mt-5"
                            />
                        </div>
            ...
            <Dialog
                open={programDiffModalOpen}
                onOpenChange={setProgramDiffModalOpen}
            >
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle className="text-xl font-bold text-slate-900">
                            Pilih Tingkat Kesulitan Program
                        </DialogTitle>
                    </DialogHeader>

                    <div className="space-y-4">
                        <p className="text-slate-600">
                            Pilih tingkat kesulitan program yang sesuai dengan
                            kondisi dan tujuan kesehatan jantungmu.
                        </p>
            ...
            >
                <DialogContent className="sm:max-w-md">
                    ...
                    <div className="space-y-4">
                        <p className="text-slate-600">
                            Kamu memiliki program yang belum selesai. Apakah
                            kamu ingin melanjutkan program tersebut?
                        </p>
                        ...
                        <div className="flex justify-end pt-4">
                            ...
                            <Button
                                ...
                                className={`bg-rose-600 hover:bg-rose-700 text-white font-semibold rounded-md transition-all duration-300 ${
                                    isNewProgramLoading ? "cursor-not-allowed opacity-50" : ""
                                }`}
                            >
                                {isNewProgramLoading ? (
                                    <div className="flex items-center gap-2">
